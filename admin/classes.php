...
<head>
    ...
    <title>Classes | Faculty Evaluation System</title>
...
<body class="bg-green-50 min-h-screen flex flex-col lg:flex-row text-green-950">
    ...
    <main class="flex-1 flex flex-col min-w-0">
        ...
        <div class="p-4 md:p-8">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6 md:gap-8">
                <div>
                    <h1 class="text-2xl font-bold text-green-950">Class List</h1>
                    <p class="text-green-700">Manage all classes. <span class="text-xs text-green-500 font-normal block mt-1">CSV Format: Class Name</span></p>
                </div>
                <div class="flex flex-wrap gap-3 w-full md:w-auto">
                    <a href="../superadmin/export_csv.php?type=class" class="bg-green-100 text-green-800 px-4 py-2 rounded-xl font-bold hover:bg-green-200 transition-all flex items-center gap-2 text-sm flex-1 md:flex-none justify-center">
                    ...
                    <table class="w-full text-left border-collapse min-w-[400px]">
                        <thead>
                            <tr class="bg-green-50 border-b border-green-200">
                                <th class="px-6 py-4 text-sm font-bold text-green-800">ID</th>
                                <th class="px-6 py-4 text-sm font-bold text-green-800">Class Name</th>
                                <th class="px-6 py-4 text-sm font-bold text-green-800 text-right">Action</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php
                            $result = $conn->query("SELECT * FROM class_list WHERE is_deleted = 0 ORDER BY class_name ASC");
                            while($row = $result->fetch_assoc()):
                            ?>
                            <tr class="border-b border-green-100 hover:bg-green-50 transition-all">
                                <td class="px-6 py-4 text-sm text-green-700 font-mono"><?php echo $row['id']; ?></td>
                                <td class="px-6 py-4 text-sm font-bold text-green-950"><?php echo $row['class_name']; ?></td>
                                <td class="px-6 py-4 text-sm text-right whitespace-nowrap">
                                    ...
                                    <button onclick="deleteClass(<?php echo $row['id']; ?>)" class="text-red-600 font-bold hover:underline">Delete</button>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                        </tbody>
    ...
    <!-- Modal -->
    <div id="classModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 p-4">
        <div class="bg-white w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl">
            <div class="p-8 border-b border-green-100 flex justify-between items-center">
                <h2 id="modalTitle" class="text-xl font-bold text-green-950">Add New Class</h2>
                <button onclick="closeModal()" class="text-green-400 hover:text-green-950"><i data-lucide="x"></i></button>
            </div>
            <form id="classForm" class="p-8 space-y-4">
                <input type="hidden" name="id" id="class_id">
                <div class="space-y-1">
                    <label class="text-xs font-black text-green-900 uppercase">Class Name (e.g., BSIT 1-A)</label>
                    <input type="text" name="class_name" id="class_name_input" required class="w-full px-4 py-2 rounded-xl border-2 border-green-300 focus:ring-2 focus:ring-green-950 outline-none transition-all placeholder:text-green-200" placeholder="BSIT 1-A">
                </div>
                <div class="pt-4">
                    <button type="submit" class="w-full bg-green-950 text-white py-3 rounded-xl font-bold hover:bg-green-900 transition-all">Save Class</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('import_csv').onchange = function() {
            if (this.files[0]) {
                ...
                .then(data => {
                    if (data == 1) {
                        Swal.fire({ icon: 'success', title: 'Success!', text: 'Classes imported successfully!' }).then(() => location.reload());
                    } else {
        ...
        function openModal(data = null) {
            ...
            if (data) {
                ...
                document.getElementById('class_name_input').value = data.class_name;
            } else {
        ...
        document.getElementById('classForm').onsubmit = function(e) {
            ...
            fetch('../ajax.php?action=save_class', {
                method: 'POST',
                body: formData
            })
            .then(res => res.text())
            .then(data => {
                if (data == 1) {
                    Swal.fire({ icon: 'success', title: 'Saved!', text: 'Class has been saved.' }).then(() => location.reload());
                } else {
        ...
        function deleteClass(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('../ajax.php?action=delete_class', {
                        method: 'POST',
                        body: formData
                    })
                    .then(res => res.text())
                    .then(data => {
                        if (data == 1) {
                            Swal.fire('Deleted!', 'Class has been deleted.', 'success').then(() => location.reload());
                        } else {
