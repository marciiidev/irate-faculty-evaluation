...
<head>
    ...
    <title>Manage Admins | Faculty Evaluation System</title>
    ...
    <main class="flex-1 flex flex-col min-w-0">
        ...
        <div class="md:p-8 p-4">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-green-950">Admin Users</h1>
                    <p class="text-green-700 text-sm md:text-base">Manage administrative users who can configure the system.</p>
                </div>
                <div class="flex flex-wrap gap-3 w-full md:w-auto">
                    <a href="export_csv.php?type=admin" class="bg-green-100 text-green-800 px-4 py-2 rounded-xl font-bold hover:bg-green-200 transition-all flex items-center gap-2 text-sm flex-1 md:flex-none justify-center">
            ...
            <div class="bg-white rounded-2xl border border-green-200 shadow-sm overflow-hidden">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse min-w-[1000px]">
                        ...
                        <tbody>
                            <?php
                            $result = $conn->query("SELECT * FROM users WHERE role = 'admin' AND is_deleted = 0 ORDER BY lastname ASC");
                            ...
                            <tr class="border-b border-green-100 hover:bg-green-50 transition-all">
                                ...
                                <td class="px-6 py-4 text-sm text-right whitespace-nowrap">
                                    ...
                                    <button onclick="deleteAdmin(<?php echo $row['id']; ?>)" class="text-red-600 font-bold hover:underline">Delete</button>
                                </td>
                            </tr>
                            <?php endwhile; ?>
                        </tbody>
    ...
    <div id="adminModal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50 p-4">
        <div class="bg-white w-full max-w-lg rounded-3xl overflow-hidden shadow-2xl">
            <div class="p-8 border-b border-green-100 flex justify-between items-center">
                <h2 id="modalTitle" class="text-xl font-bold text-green-950">Add New Admin</h2>
                <button onclick="closeModal()" class="text-green-400 hover:text-green-950"><i data-lucide="x"></i></button>
            </div>
            <form id="adminForm" class="p-8 space-y-4" enctype="multipart/form-data">
                <input type="hidden" name="id" id="admin_id">
                <input type="hidden" name="role" value="admin">

                <div class="flex flex-col items-center gap-2 mb-4">
                    <div class="w-20 h-20 rounded-full bg-green-50 border-2 border-dashed border-green-200 flex items-center justify-center overflow-hidden relative group" id="admin-avatar-container">
                        <i data-lucide="user" class="w-8 h-8 text-green-200" id="admin-avatar-placeholder"></i>
                        <img id="admin-avatar-preview" class="w-full h-full object-cover hidden">
                        <label for="admin_avatar" class="absolute inset-0 flex items-center justify-center bg-black/40 text-white opacity-0 group-hover:opacity-100 transition-all cursor-pointer">
                            <i data-lucide="camera" class="w-5 h-5"></i>
                        </label>
                    </div>
                    <input type="file" name="avatar" id="admin_avatar" class="hidden" accept="image/*" onchange="previewAdminAvatar(this)">
                ...
                <div class="pt-4">
                    <button type="submit" class="w-full bg-green-950 text-white py-3 rounded-xl font-bold hover:bg-green-900 transition-all">Save Admin</button>
                </div>
            </form>
        </div>
    </div>
